        ...
        <div className={`${showModal ? 'fixed' : 'hidden'} z-10 inset-0 overflow-y-auto`}>
            ...
            <div className="flex items-center justify-center min-h-screen  text-center p-4 sm:block">
                ...
                <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden w-full shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div className="p-6">
                        <h3 className="text-lg font-medium leading-6 text-gray-900 mb-2">Address Selection</h3>
                        <div className='flex justify-around gap-3'>
                            <button
                                className={`p-2 ${selectedOption === 'New' ? 'bg-green-400 rounded-md' : 'bg-white border border-green-500 rounded-md'} text-sm text-gray-700`}
                                onClick={() => setSelectedOption('New')}
                            >
                                New Address
                            </button>
                            <button
                                className={`p-2 ${selectedOption === 'My' ? 'bg-green-400 rounded-md' : 'bg-white border border-green-500 rounded-md'} text-sm text-gray-700`}
                            ...
                            <div className="mt-6">
                                <div className="mb-4">
                                    <div className=' flex justify-between my-1'>
                                        <p className="text-sm font-medium text-gray-700 ">Input New Address</p>
                                        <div>
                                            <button className=' p-1 bg-green-600 rounded-md text-xs font-bold text-white'>Get Current</button>

                                        </div>
                                    </div>
                                    <div className='text-center inline-flex w-full my-1'>

                                    <input type="text" value={contactPerson} onChange={(e) => setContactPerson(e.target.value)} placeholder='Contact Person' className='appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-green-500 focus:border-green-500 focus:z-10 sm:text-sm' />
                                    </div>
                                    <div className='text-center inline-flex w-full my-1'>

                                    <input type="text" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder='Phone Number' className='appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-green-500 focus:border-green-500 focus:z-10 sm:text-sm' />
                                    </div>
                                    ...
                                    <div className='text-center inline-flex w-full my-1'>
                                        ...
                                        <button onClick={handleSearch} className=' bg-green-700 text-white rounded-r-md px-2'> Search </button>
                                    </div>


                                    <div className="  bg-gray-100 p-2 rounded-md max-h-40 overflow-y-auto">
                                        {searchResults.map((result, index) => (
                                            <div key={index} className="border border-gray-300 rounded-md p-4 mb-1">
                                                <p className="text-[0.8em] font-semibold">{result.display_name}</p>
                                                ...
                                                <button  onClick={() => handleAddressSelect(result, contactPerson, phoneNumber)}
                                                className='mt-2 border text-[0.8em] border-green-700 font-bold text-green-700  py-2 px-4 rounded focus:outline-none focus:shadow-outline'> SELECT </button>
